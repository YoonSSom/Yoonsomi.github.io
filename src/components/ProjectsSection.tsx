...
const projects: Project[] = [
  ...
  {
    id: "commercial",
    title: "디저트 커머스 웹사이트 (SEO & 데이터 기반 성장)",
    summary: "SEO와 데이터 분석을 통해 광고비 없이 월 방문자 2,400명과 전환율 3.2%를 달성한 그로스형 커머스 구축",
    problemGoal: [
    ...
    myRole: {
      planning: [
        "Google Search Console 기반 롱테일 키워드 발굴 및 검색 의도 기반 콘텐츠 구조 설계",
        "CTR 낮은 페이지 메타태그 개선 → 검색 노출 증가 및 유기적 트래픽 확보",
        "GA4 기반 '유입 → 상품 조회 → 장바구니 → 구매' 퍼널 설계 및 이탈 구간 분석",
        "KPI 대시보드 구축 (주간 단위) 및 전환율 추적",
        "GTM 기반 주요 행동 이벤트 정의 (CTA 클릭, 상품 조회 등) 및 전환 이벤트 구조 설계",
        "A/B 테스트 백로그 관리: CTA 문구/이미지/배치 실험, 콘텐츠 유형별 성과 비교 (30건+ 실험 축적)",
        "모바일 중심 UX 설계 및 CTA 버튼 가시성·구매 유도 UI 구조 설계",
      ],
      development: [
        "Next.js(SSG) 기반 검색엔진 친화적 페이지 구조 설계 및 구현",
        "Core Web Vitals 개선 (LCP 2.1초 달성) 및 콘텐츠 중심 URL 구조 설계",
        "GA4 + GTM 이벤트 트래킹 설계 및 구축",
        "Tailwind CSS 기반 반응형 UI 스타일링 및 React 컴포넌트 기반 UI 설계",
      ],
      other: [
    ...
    techStack: ["Next.js (SSG)", "React", "Tailwind CSS", "GA4", "GTM", "Google Search Console"],
    results: [
