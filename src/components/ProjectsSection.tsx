...
const projects: Project[] = [
  {
    ...
    fullDescription: "코로나19 이후 비대면 의료 서비스의 필요성이 증가함에 따라, 환자들이 병원을 직접 방문하지 않고도 수술 동의서에 전자서명을 할 수 있는 웹 기반 서비스를 개발했습니다. React와 TypeScript를 활용하여 프론트엔드를 구축하고, 전자서명 API를 연동하여 법적 효력이 있는 서명 시스템을 구현했습니다. 환자 인증, 문서 열람, 서명, 제출까지의 전 과정을 모바일에서도 원활하게 진행할 수 있도록 반응형 UI를 적용했습니다.",
    tags: [
  ...
  },
  {
    ...
    fullDescription: "Kaggle에서 주최한 HuBMAP + HPA 대회에 참가하여 5개 장기(신장, 대장, 비장, 전립선, 폐)의 조직 이미지에서 기능적 조직 단위(FTUs)를 정확하게 분할하는 모델을 개발했습니다. U-Net 기반의 Segmentation 모델을 구현하고, 다양한 Data Augmentation 기법과 앙상블 전략을 적용하여 성능을 최적화했습니다. 의료 영상의 특성을 고려한 전처리와 후처리 파이프라인을 구축했습니다.",
    tags: [
  ...
  },
  {
    ...
    fullDescription: "한국을 방문한 외국인 관광객들이 식당에서 겪는 언어 장벽 문제를 해결하기 위한 OCR 기반 메뉴판 번역 시스템을 개발했습니다. Tesseract OCR을 활용하여 한국어 텍스트를 인식하고, Google Translate API를 연동하여 영어, 중국어, 일본어 등 다국어로 번역하는 기능을 구현했습니다. 메뉴판 이미지의 전처리(노이즈 제거, 이진화)를 통해 인식률을 향상시켰습니다.",
    tags: [
  ...
  },
  {
    ...
    fullDescription: "졸음 운전으로 인한 교통사고를 예방하기 위해 실시간 졸음 감지 시스템을 개발했습니다. YOLOv4 객체 탐지 모델을 커스텀 학습하여 운전자의 눈 상태(떠있음/감김)를 실시간으로 분류합니다. 일정 시간 이상 눈을 감고 있으면 경고음을 발생시켜 운전자에게 알립니다. Darknet 프레임워크를 활용하여 GPU 가속을 적용했으며, 웹캠을 통한 실시간 처리가 가능하도록 최적화했습니다.",
    tags: [
  ...
  },
  {
    ...
    fullDescription: "공유 차량 서비스에서 반납 시 차량 파손 여부를 자동으로 감지하는 AI 모델을 개발했습니다. DeepLab V3+ 기반의 Semantic Segmentation 모델을 학습하여 스크래치, 찌그러짐, 깨짐 등의 파손 유형을 픽셀 단위로 분류합니다. 다양한 조명 조건과 촬영 각도에서도 안정적으로 동작하도록 Data Augmentation을 적용했으며, 실제 쏘카 차량 이미지 데이터셋을 활용하여 학습했습니다.",
    tags: [
  ...
  },
  {
    ...
    fullDescription: "무신사 쇼핑몰의 상품 리뷰 데이터를 수집하여 감성 분석 모델을 개발했습니다. KoBERT를 Fine-tuning하여 리뷰의 긍정/부정/중립 감정을 분류하고, 별점을 예측하는 회귀 모델도 함께 구현했습니다. 데이터 불균형 문제를 해결하기 위해 Back-translation, EDA(Easy Data Augmentation) 등의 데이터 증식 기법을 적용했습니다. 분석 결과를 시각화하여 시즌별 트렌드 인사이트를 도출했습니다.",
    tags: [
