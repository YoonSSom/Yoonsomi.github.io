...
import { ExternalLink, Calendar, Users, Wrench, CheckCircle2, Circle, Target, TrendingUp, GitBranch, FileText, Lightbulb, BarChart3 } from "lucide-react";
...
const ProjectCard = ({ 
  ...
  return (
    <div
      ref={ref}
      onClick={onClick}
      className={`group relative overflow-hidden rounded-2xl bg-card border border-border hover:border-primary hover:shadow-xl hover:shadow-primary/20 hover:-translate-y-2 transition-all duration-500 cursor-pointer ${
        isVisible 
            ...
            <div className="flex flex-col max-h-[90vh]">
              ...
              <div className="mb-6 p-4 rounded-lg bg-gradient-to-r from-primary/10 to-primary/5 border border-primary/20">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="text-sm font-semibold mb-1 text-primary">해결한 문제</h4>
                    <p className="text-sm text-foreground/90">{selectedProject.problem}</p>
                  </div>
                </div>
              </div>
              ...
              {selectedProject.metrics && selectedProject.metrics.length > 0 && (
                <div className="mb-6">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h4 className="text-sm font-semibold text-muted-foreground">성과 지표</h4>
                  </div>
                  <div className="grid grid-cols-3 gap-3">
                    {selectedProject.metrics.map((metric, idx) => (
                      <div key={idx} className="text-center p-3 rounded-lg bg-muted/50 border border-border/50">
              ...
              {selectedProject.myRole && (
                <div className="mt-4 p-3 rounded-lg bg-muted/30 border border-border/50">
                  <p className="text-xs text-muted-foreground mb-1">나의 역할</p>
                  <p className="text-sm font-medium">{selectedProject.myRole}</p>
                </div>
              )}
              ...
              {selectedProject.decisions && selectedProject.decisions.length > 0 && (
                <div className="mt-6">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h4 className="text-sm font-semibold text-muted-foreground">설계 결정</h4>
                  </div>
                  <div className="space-y-3">
                    {selectedProject.decisions.map((decision, idx) => (
                      <div key={idx} className="p-3 rounded-lg bg-muted/30 border border-border/50">
                        <p className="text-sm font-medium text-primary mb-1">{decision.question}</p>
                        <p className="text-sm text-foreground/80">{decision.answer}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {/* Collaboration */}
              {selectedProject.collaboration && selectedProject.collaboration.length > 0 && (
                <div className="mt-6">
                  <div className="flex items-center gap-2 mb-3">
                    <GitBranch className="w-4 h-4 text-primary" />
                    <h4 className="text-sm font-semibold text-muted-foreground">협업 & 문서화</h4>
                  </div>
                  <ul className="space-y-2">
                    {selectedProject.collaboration.map((item, idx) => (
              ...
              {/* Growth - What I Learned */}
              {selectedProject.growth && (
                <div className="mt-6">
                  <div className="flex items-center gap-2 mb-3">
                    <TrendingUp className="w-4 h-4 text-primary" />
                    <h4 className="text-sm font-semibold text-muted-foreground">성장 포인트</h4>
                  </div>
                  <div className="p-4 rounded-lg bg-gradient-to-r from-primary/5 to-transparent border-l-2 border-primary">
